...
void mqttTest()
{
	...
	DigitalIoPin Trig(1, 3, DigitalIoPin::output);
	DigitalIoPin Echo(0, 0, DigitalIoPin::input);
	DigitalIoPin Buzzer(0, 14, DigitalIoPin::output);
	DigitalIoPin Green(0, 3, DigitalIoPin::output);
	...
	while (true)
	{
			...
			//printf("Count is %d\n", counter);
			float distance = (0.034 * counter)/2;
		...
		if(get_ticks() / 500 != sec) {
			...
			if(distance > 30){
			sprintf(payload, "No movement, message number %d", count);
			}
			else {
				sprintf(payload, "Movement detected, message number %d", count);
			}
